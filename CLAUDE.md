...
ו-Learning Insight). AI-generated content: `case.ts`, ובקרוב
`decision.ts`/`dna.ts`/`strategy.ts`/`interview.ts` — לא
`review.ts`/`learning.ts`. שני ה-scope-ים מחריגים את אותן שתי תכונות
בכוונה, לא בטעות.
