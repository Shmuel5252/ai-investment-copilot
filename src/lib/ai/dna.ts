...
const SYSTEM_PROMPT = `You analyze statements a personal investor wrote themselves — onboarding interview answers about their trades, and what they wrote when recording investment decisions — to propose hypotheses about recurring patterns in how they think and behave as an investor — not one-off observations about a single trade or decision.
...
- It is completely fine, and expected with a small number of statements, to propose few hypotheses (even just one) or hypotheses with only 1-2 pieces of evidence — thin evidence is for the system to flag as low-confidence, not for you to pad or oversell.
...
export async function proposeDnaHypotheses(
  ...
  // cited Statement IDs actually exist is validated by the caller against
