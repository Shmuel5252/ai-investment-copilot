...
- **מקורות הראיה (Evidence Reach V1, 2026-09-25 — ר' §2.11):** ה-AI מקבל
  **הצהרות שהמשקיע כתב בעצמו** בלבד, כל אחת עם Statement ID ותווית מקור:
  `InterviewAnswer` אפקטיביות **וגם** שלושת הטקסטים שכתב בזמן ההחלטה לתוך
  ה-`DecisionSnapshot` הקפוא (`user_reasoning_text`, `risks_considered_text`,
  `exit_conditions_text`). לעולם לא: טקסט AI (Personal Fit, סינתזה,
  bull/bear, הערכת זמן-אמת, תחזיות, נרטיב/ממדי Review, תיאורי ראיה), Later
  Context, Outcome/ביצוע/P&L. הצהרת החלטה היא ראיה ל**תהליך** (מה חשב/שקל/תכנן),
  לא לכך שצדק. עצמאות: OD-2 (`src/lib/evidence/decision-cases.ts`).
...
- **מערכת:** Declared מהצהרות ראיון (AI מחלץ, משתמש מאשר). Observed מאותו
  מנוע Evidence כמו DNA — מ-2026-09-25 גם מהצהרות מזמן החלטה, אותם מקורות,
  אותה עצמאות ואותן החרגות (OD-4, §2.11); Declared ללא שינוי. סט קבוע של
  עקרונות סיכון בסיסיים — קוד, מסומן ככזה.
...
- **מערכת:** קיבוץ החלטות — קוד. סינתזת משמעות — AI. סף ראיות מינימלי —
  ...
  **חדשה** (לא עדכון של קיימת — אין דרך חד-משמעית לבחור לאיזו קיימת
  לצרף). **OD-3 (Evidence Reach V1, 2026-09-25; הוקשח ב-review 2026-09-25):**
  ההסכמה עצמה והתובנה עצמה **אינן** ראיה (0 מקרים). ההשערה החדשה **נושאת** את
  ההחלטות שהגרסה שאושרה ציטטה (`provenance_json.citedReviews` של אותה גרסה —
  ה-stance שנתנה לכל Review; לגרסאות ישנות — שורות ה-Evidence), מקרה אחד לכל
  החלטה-מקור (כמה Reviews של אותה החלטה = מקרה אחד). **Review שציטט החלטה אינו
  ביסוס טקסטואלי של ההשערה החדשה:** כל אחת מהצהרות ההחלטה שהמשקיע כתב
  (נימוק/סיכונים/תנאי יציאה) נבדקת מול ניסוח ההשערה עם ה-stance של התובנה
  **באותו שער grounding שכל ציטוט DNA עובר** (`checkEvidenceGrounding`,
  fail-closed); רק הצהרות `supported` הופכות לשורות Evidence
  (`decision_id` + הסוג שביסס). החלטה שאף הצהרה שלה לא מבססת — לא נישאת; אין
  אף הצהרה מבוססת — אין השערה (BAD_REQUEST, בלי Correction). הספירה דרך
  ה-resolver המשותף תחת OD-2 (סף ≥3 ללא שינוי; סתירה לא מעלה ביטחון). Reviews
  הופכים לראיית DNA **רק** דרך תובנה שאושרה. הסכמה חוזרת = replay לפני כל
  קריאת AI (provenance `carriedFromLearningInsightId`). **זהות תובנה =
  (investor, family) — OD-R1, הוקפא 2026-09-25:** זרם סינתזה **אחד** לכל
  משפחה ב-V1 (כמה תובנות עצמאיות באותה משפחה — נדחה ל-Learning V2). גרסאות
  append-only; גרסה חדשה **רק** כשטביעת מצב-הראיה האפקטיבי משתנה
  (`src/lib/learning/evidence-fingerprint.ts`: `lef-v1:` + רשומות
  `decisionId|reviewId|stance` ממוינות — מזהים ו-stances בלבד, בלי ניסוח, בלי
  סדר, בלי זמנים): החלטה חדשה מצוטטת, החלטה שהסינתזה כבר לא מצטטת, היפוך
  stance, החלפת Review — כולם גרסה; **ניסוח בלבד** מעל אותו מצב — לא גרסה, לא
  זהות, לא החלפת הניסוח השמור, לא ראיה, לא מקרה: מדווח כ-`unchanged` עם
  `wordingDiffers`/`proposedStatementText`. כל גרסה שומרת ב-provenance את
  הרשומות ואת הטביעה שהצדיקו אותה; שורות Evidence per-identity, append-only,
  לפי (review, stance). נעילת advisory למשקיע; ריצות מקבילות עם שינויים
  מהותיים מסודרות בסדר גרסאות דטרמיניסטי.
- **Done:** תובנה אמיתית אחת לפחות, traceable, תגובת המשתמש משפיעה על
  גרסה עתידית של DNA (דרך Evidence שמצטט את החלטות-המקור של ה-Insight — OD-3).
...
### 2.11 Evidence Reach (V1, 2026-09-25)
נמצא בפועל (ריצה חיה, קריאה בלבד): 10/10 השערות DNA ו-6/6 עקרונות נצפים
insufficient_evidence; ה-AI ראה 2 מוצהרים + 4 מאומתים בלבד; ~2,300 תווים
שהמשקיע כתב בזמן 3 החלטות מעולם לא הוזנו כראיה; הסכמה לתובנת Learning יצרה
השערה עם S=1 (הסכמה = ראיה) שמיד הוחרגה. הכרעות Owner קפואות (OD-1..OD-4):
- **OD-1 מקורות:** DNA/Strategy נצפית קוראים תשובות ראיון אפקטיביות + הצהרות
  מזמן החלטה (נימוק/סיכונים/תנאי יציאה), **מתויגות מקור**, verbatim (§2.3).
  אין Statement ID לשום טקסט AI או פוסט-החלטה — ולכן אין דרך לצטט אותו;
  `parseStatementId` + `hasStatement` מול הרשומות של אותו משקיע בלבד.
  Later Context נשאר זמין ל-Review בלבד.
- **OD-2 עצמאות של החלטה** (`decision-cases.ts`, טהור; נטען מרשומות בלבד):
  עובדת `executed` אפקטיבית (§2.10) ⇒ ההחלטה + ה-episode של אותה עסקה =
  מקרה **אחד**; לעולם לא מיזוג לפי טיקר/תאריך/סכום/כוונה; מועמדות ביצוע
  (אותו טיקר, צד תואם, מיום ההחלטה−1 ואילך — superset fail-closed) שאף אחת
  לא סווגה ⇒ **UNRESOLVED**: לא נספר לאף צד (`unresolvedDecisionIds` ב-basis,
  review-only); PASS/HOLD ובלי מועמדות ⇒ מקרה משלה; `unrelated` מסווג ולא
  ממזג; כמה עסקאות מבוצעות = עדיין החלטה אחת (label `decision:<id>`).
  מדיניות `independence-policy-v2` (`decisionCases: evidence-source-v1`,
  `candidateDayTolerance: 1` — חלון המועמדות מתחיל ביום ה-UTC של ההחלטה פחות
  יום אחד, superset של רשימת המועמדות התלויה באזור-זמן של המוצר; יכול רק
  **להרחיב** את מה שדורש סיווג, לעולם לא להסיק ביצוע/למזג/לספור);
  גרסאות שנספרו קודם לא נכתבות מחדש (בלי backfill). **OD-R2 (הוקפא
  2026-09-25):** החלטה אחת = מקרה עצמאי אחד; אם המשקיע סימן `executed`
  עסקאות משני episodes חשבונאיים — המקרה נושא את **איחוד** ה-labels של שני
  ה-episodes (לעולם לא שני מקרים), **רק** דרך עובדות `executed` אפקטיביות
  שהמשקיע אישר — לעולם לא לפי טיקר/קרבת תאריך/חפיפת פוזיציה/סכום/כוונה;
  האיחוד רשאי לשנות גבולות סתירה כי המשקיע אישר שה-episodes שייכים להחלטה
  אחת. ה-basis משחזר: החלטה → עובדות executed (`executed_fact` reasons) →
  labels של episodes → קבוצת האיחוד. **OD-R5:** `candidateDayTolerance = 1`
  הוקפא ב-`independence-policy-v2` ונרשם בכל basis.
- **OD-3 Learning→DNA:** §2.8. **OD-4:** §2.4.
- **Provenance:** `provenance_json` על גרסאות DNA/Strategy/Learning חדשות
  (generator, model, prompt contracts, evidence-source contract, מדיניות
  עצמאות, זמן, code version); שורות ישנות NULL, לעולם לא backfill.
- **שקיפות (`evidence.reach`, קריאה בלבד):** לכל השערה/עיקרון נצפה — מקורות
  לפי סוג, "ה-AI משתמש/לא משתמש בזה כרגע" (סף, לא "שגוי"), החלטות מצוטטות
  לא-פתורות, ומרחק לרמה הבאה **רק** כשטבלת הסף אומרת זאת בדיוק
  (`distanceToNextTier`, C קבוע): "נדרשים עוד X מקרים עצמאיים תומכים" — ראיה
  נוספת על אותו אירוע אינה מקרה.
- **"מה הצעד הבא" (`evidence.nextActions`)** בדשבורד ליד ה-Monitoring — קטלוג
  דטרמיניסטי FACT→REASON→DESTINATION שנעלם כשבוצע, סדר קבוע: סיווג מועמדות
  ביצוע (עד אז ההצהרות לא נספרות), Review להחלטה ללא Review (תאריך שעבר, או
  ללא תאריך ומעל 30 יום), קביעת תאריך Review, תנאי שקילה-מחדש שהגיע מועדו,
  Case שנתקע (≥14 יום), אפיזודות ללא נימוק, יצירה מחדש **רק** כשיש הצהרות שאף
  claim לא מצטט **וגם** הצהרה שנוספה אחרי ריצת ה-generate האחרונה (או שאין
  ריצה עם provenance) — ריצה שכבר ראתה את כל ההצהרות ולא ציטטה חלקן היא
  שיפוט המודל, לא reach חסר. **OD-R4 (הוקפא 2026-09-25):**
  `STALLED_CASE_DAYS = 14`, `REVIEW_WITHOUT_HORIZON_NUDGE_DAYS = 30` — קבועי
  **מדיניות דשבורד בלבד**, ניתנים לשינוי, **לא** נטענים כאופטימליים אמפירית;
